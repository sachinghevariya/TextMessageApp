...
) : BaseDifferAdapter<Conversation, ConversationAdapter.ViewHolder>(diffCallback = DiffCallback()),
...
//    val statusBarColor = typedArrayBg.getColor(0, 0)
    ...
    override fun onBindViewHolder(
        ...
        if (AdsUtility.isNetworkConnected(baseActivity) && AdsUtility.config.adMob.nativeAd.isNotEmpty()) {
            if (item?.threadId != 0L && item?.message != null && position > 0) {
    ...
    private fun setupView(
        ...
        view.apply {
            ...
            conversationDate.apply {
                text = msg.date.formatDateOrTime(
                    context, true,
...
//                    clearSelection()
    ...
    private fun checkReadBtnVisibility(menu: Menu) {
        val selectedConversations = selectedItems
        val allUnread = selectedConversations.all { !it.message?.read!! }
        val allRead = selectedConversations.all { it.message?.read!! }
